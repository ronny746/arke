...
import { PenTool, CheckCircle, Plus } from 'lucide-react';
...
export default function StudentPractice() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [filtersRes, historyRes] = await Promise.all([
        ...
        studentAPI.getPracticeHistory({ sessionType: 'PRACTICE' })
      ]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to generate Practice. Try broader criteria.');
    } finally {
      setGenerating(false);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in p-6">
      <PageHeader
        ...
        subtitle="Practice questions one by one with immediate solutions"
        breadcrumbs={['Home', 'Practice']}
      />
      ...
      {loading ? (
        ...
        <div className="flex flex-col">
          ...
          <Card className="p-6 mb-8 border-primary-100 bg-gradient-to-br from-primary-50/30 to-white shadow-sm rounded-2xl">
            <h3 className="font-bold text-lg mb-4 text-gray-800 flex items-center"><Plus className="w-5 h-5 mr-2 text-primary-600"/> Generate New Practice Session</h3>
          ...
          <div className="space-y-4">
            <h3 className="font-bold text-xl text-gray-900 border-b pb-2">Your Practice History</h3>
            ...
            {history.length === 0 ? (
              <div className="text-center p-12 bg-gray-50 rounded-2xl border border-gray-100 text-gray-500 shadow-sm">
                ...
                <p className="text-lg font-medium text-gray-700">You haven't generated any Practice Sessions yet.</p>
                ...
                {history.map(session => (
                  <Card key={session._id} className="p-5 hover:shadow-md transition-all hover:border-primary-300 rounded-2xl bg-white border border-gray-100">
                    ...
                    <Button 
                      ...
                      onClick={() => router.push(`/student/practice/${session._id}/play`)}
                    >
                      {session.status === 'COMPLETED' ? 'View Results' : 'Resume Practice'}
                    </Button>
                  </Card>
                ))}
              </div>
