...
export default function AdminStudentPerformancePage({ params }) {
  const router = useRouter();
  const unwrappedParams = use(params);
  const studentId = unwrappedParams.id;

  const handleExamClick = (examId, submissionId) => {
    // Navigate to admin exam analysis
    router.push(`/admin/exams/${examId}/results/${submissionId}/analysis`);
  };

  const handleDppClick = (sessionId) => {
    router.push(`/admin/dpp/${sessionId}/analysis`);
  };
