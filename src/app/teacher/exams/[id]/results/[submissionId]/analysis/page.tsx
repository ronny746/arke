...
import { teacherAPI as adminAPI } from '@/api/teacher.js';
...
export default function AdminExamAnalysis() {
  ...
  const { submission, detailedQuestions, totalMarks, score } = data;
  ...
  return (
    <div className="space-y-6 animate-fade-in p-6">
      <div className="flex items-center gap-4 mb-4">
        <Button variant="outline" icon={ArrowLeft} onClick={() => router.push(getBackPath())}>Back to Results</Button>
      </div>

      <PageHeader
        title="Exam Analysis"
        subtitle={`Detailed breakdown for ${submission?.student?.firstName ? `${submission.student.firstName} ${submission.student.lastName}` : submission?.student?.name || submission?.publicUser?.name || 'Student'}`}
      />
      ...
      <Card className="overflow-hidden mt-6">
        ...
        <div className="p-6 space-y-8 bg-gray-50">
          {(detailedQuestions || data.details || []).filter(q => {
            const subjectName = (q.subject && typeof q.subject === 'object') ? q.subject.name : (q.subject || 'General');
            return activeSubject ? subjectName === activeSubject : true;
          }).map((q, index) => {
