...
export default function Messages() {
  ...
  useEffect(() => {
    if (showNewChat && teachersList.length === 0) {
      fetchTeachers();
    }
  }, [showNewChat]);
  ...
  const fetchRooms = async () => {
    try {
      const res = await parentAPI.getChatRooms();
  ...
  const fetchTeachers = async () => {
    try {
      const res = await parentAPI.getUsers({ role: 'teacher' });
      const teachers = Array.isArray(res.data?.data) ? res.data.data : (res.data?.data?.users || []);
  ...
  const selectRoom = async (room) => {
    ...
    try {
      const res = await parentAPI.getChatMessages(room._id || room.id);
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await parentAPI.sendMessage(selectedChat._id || selectedChat.id, { content: messageText });
  ...
  const createChat = async () => {
    ...
    setLoading(true);
    try {
      const res = await parentAPI.createChatRoom({
        type: 'DIRECT',
        participantIds: [selectedTeacher]
      });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Messages" subtitle="Communicate with your child's teachers" breadcrumbs={['Home', 'Messages']} />
        ...
        <ModalBody className="space-y-4">
          <FormField label="Select Teacher">
            <Select value={selectedTeacher} onChange={e => setSelectedTeacher(e.target.value)}>
              ...
              {teachersList.map(t => <option key={t._id || t.id} value={t._id || t.id}>{t.firstName} {t.lastName}</option>)}
            </Select>
          </FormField>
          {teachersList.length === 0 && <p className="text-xs text-surface-500">No teachers found in the system.</p>}
        </ModalBody>
        <ModalFooter>
