...
import { Plus, Edit, Trash2, Eye, CheckCircle, FileText, Download } from 'lucide-react';
...
const getPreviewUrl = (url) => {
  ...
  // For docs, pdfs, use Google Docs Viewer
...
export default function Assignments() {
  ...
  const fetchAssignments = async () => {
    setLoading(true);
    try {
      const res = await teacherAPI.getViewAssignments();
  ...
  const columns = [
    ...
    { header: 'Max Marks', accessorKey: 'maxMarks', cell: (r) => <span className="font-medium">{r.maxMarks}</span> },
    {
      ...
      cell: (r) => (
        <button 
          onClick={(e) => { e.stopPropagation(); setShowGrade(r); fetchSubmissions(r._id || r.id); }}
          className="text-sm text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 font-medium hover:underline px-2 py-1 bg-primary-50 dark:bg-primary-900/20 rounded-md transition-colors"
        >
          View Submissions
        </button>
      ),
  ...
  const fetchSubmissions = async (assignmentId) => {
    ...
    try {
      const res = await teacherAPI.getViewSubmissions(assignmentId);
      ...
      await Promise.all(
        submissionsList.map(sub => {
          ...
          if (marks !== undefined && marks !== '') {
            return teacherAPI.gradeSubmission(sub._id || sub.id, { marksObtained: Number(marks) });
          }
                  ...
                  <div key={sub._id || sub.id} className="flex flex-col gap-2 p-3 bg-surface-50 dark:bg-surface-700/50 rounded-xl">
                    <div className="flex items-center gap-4">
                      ...
                      {sub.attachments && sub.attachments.length > 0 && (
                        <button 
                          onClick={() => setPreviewId(previewId === (sub._id || sub.id) ? null : (sub._id || sub.id))}
                          className="text-xs text-primary hover:bg-primary-50 px-2 py-1 rounded-md font-medium flex items-center gap-1 transition-colors"
                        >
                          <FileText size={14} /> {previewId === (sub._id || sub.id) ? 'Hide File' : 'Preview File'}
                        </button>
                      )}
                    ...
                    {previewId === (sub._id || sub.id) && sub.attachments && sub.attachments.length > 0 && (
                      <div className="mt-2 w-full bg-surface-100 dark:bg-surface-900 rounded-xl overflow-hidden border border-surface-200 dark:border-surface-700">
                        <iframe 
                          src={getPreviewUrl(sub.attachments[0])} 
                          className="w-full h-64 sm:h-80" 
                          title="Submission Preview"
                        />
                        <div className="flex justify-end p-2 bg-surface-50 dark:bg-surface-800">
                          <a href={sub.attachments[0]} target="_blank" rel="noreferrer" className="text-xs font-medium text-primary hover:underline flex items-center gap-1">
                            <Download size={12} /> Open/Download in New Tab
                          </a>
                        </div>
                      </div>
                    )}
