...
export default function ExamPlayer() {
  ...
  }, [id, violations, submitting, navigate]);
  ...
  const subjects = [...new Set(examState.questions.map(q => q.subject || 'General'))];
  const questionsInSubject = examState.questions.filter(q => (q.subject || 'General') === activeSubject);
            ...
            {subjects.map(sub => (
              <button
                ...
                onClick={() => {
                  ...
                  const firstIdx = examState.questions.findIndex(q => (q.subject || 'General') === sub);
                 ...
                 <div className="flex gap-3">
                   <Button 
                    ...
                    onClick={() => {
                      ...
                      setActiveSubject(examState.questions[nextIdx].subject || 'General');
                    }}
                   >
                     Previous
                   </Button>
         ...
         <div className="p-4 border-b bg-gray-50 flex flex-col items-center">
          ...
          <div className="p-4 flex-1 overflow-y-auto space-y-6">
            {subjects.map(subject => {
              const subjectQuestions = examState.questions.map((q, i) => ({ q, i })).filter(({ q }) => (q.subject || 'General') === subject);
