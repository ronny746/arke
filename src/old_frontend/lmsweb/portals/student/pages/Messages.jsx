...
export default function Messages() {
  ...
  const fetchRooms = async () => {
    try {
      const res = await studentAPI.getChatRooms();
  ...
  const fetchTeachers = async () => {
    try {
      const res = await studentAPI.getUsers({ role: 'teacher' });
  ...
  const selectRoom = async (room) => {
    ...
    try {
      const res = await studentAPI.getChatMessages(room._id || room.id);
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await studentAPI.sendMessage(selectedChat._id || selectedChat.id, { content: messageText });
  ...
  const createChat = async () => {
    ...
    setLoading(true);
    try {
      const res = await studentAPI.createChatRoom({
        type: 'DIRECT',
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Messages" subtitle="Communicate with your teachers" breadcrumbs={['Home', 'Messages']} />
