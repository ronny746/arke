...
export default function AdminExamAnalysis() {
  ...
  const fetchAnalysis = async () => {
    try {
      setLoading(true);
      const res = await adminAPI.getAdminExamAnalysis(submissionId);
  ...
  const { details, score, totalTime, submittedAt, student } = data;

  const getBackPath = () => {
    if (window.location.pathname.startsWith('/teacher')) return `/teacher/exams/${id}/results`;
    if (window.location.pathname.startsWith('/super-admin')) return `/super-admin/exams/${id}/results`;
    return `/admin/exams/${id}/results`;
  };

  return (
    <div className="space-y-6 animate-fade-in p-6">
      <div className="flex items-center gap-4 mb-4">
        <Button variant="outline" icon={ArrowLeft} onClick={() => navigate(getBackPath())}>Back to Results</Button>
      </div>

      <PageHeader
        title="Student Exam Analysis"
        subtitle={`Detailed breakdown for ${student?.firstName ? `${student.firstName} ${student.lastName}` : student?.name || 'Student'}`}
      />
            ...
            return (
              <div key={q.questionId} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    {!isAttempted ? (
                      ...
                      <span className="px-2 py-1 bg-error-100 text-error-700 text-xs rounded-full font-bold flex items-center gap-1"><XCircle className="w-3 h-3"/> Incorrect ({q.marksObtained})</span>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-3 pl-12">
                  {q.options.map((opt) => {
                    const isSelected = q.userAnswer && q.userAnswer.selectedOptionId === opt._id;
