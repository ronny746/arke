...
import { Plus, Edit, Trash2, Eye, Download, Upload, GraduationCap, ArrowRight } from 'lucide-react';
...
export default function Students() {
  ...
  const handleAdd = async () => {
    if (!form.firstName || !form.email || !form.password || !form.rfid) {
      toast.error('First Name, Email, Password, and RFID are required');
      return;
    }
    setLoading(true);
    try {
      ...
      <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} size="xl">
        ...
        <ModalBody>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormField label="RFID" required><Input placeholder="RFID Number" value={form.rfid} onChange={e => setForm(f => ({ ...f, rfid: e.target.value }))} /></FormField>
        ...
        <Modal isOpen={!!showEdit} onClose={() => setShowEdit(null)} size="xl">
          ...
          <ModalBody>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FormField label="RFID" required><Input value={showEdit.rfid || ''} onChange={e => setShowEdit(f => ({ ...f, rfid: e.target.value }))} /></FormField>
