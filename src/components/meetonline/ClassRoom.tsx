...
export default function ClassRoom({ user, token, roomCode, roomType, mobile, onLeave }: ClassRoomProps) {
  ...
  // In a live class, students should only see the teacher(s) and their screen shares
  if (roomType === 'live_class' && user.role === 'student') {
    allTiles = allTiles.filter(t => t.role !== 'student');
  }
      ...
      >
        {showVideo ? (
          <video
            ...
            className={`w-full h-full object-contain ${isLocal && !isScreen ? 'transform scale-x-[-1]' : ''}`}
          />
        ) : (
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-3.5 z-10">
          ...
          <div className="flex items-center space-x-1.5">
            <span className="text-xs font-semibold text-white drop-shadow-sm truncate max-w-[85%]">
              ...
              <span className="ml-2 px-1.5 py-0.5 rounded bg-brand-500/80 text-[9px] uppercase tracking-wider">{role}</span>
            </span>
            {isSpeaking && (
        ...
        <div className="absolute bottom-2.5 left-2.5 bg-slate-900/60 backdrop-blur-md px-2 py-0.5 rounded text-[10px] font-medium text-white flex items-center space-x-1 opacity-100 group-hover:opacity-0 transition-opacity">
          ...
          <span className="ml-1 px-1 py-[1px] rounded bg-brand-500/60 text-[8px] uppercase">{role}</span>
          {isSpeaking && <span className="w-1 h-1 rounded-full bg-green-50 animate-pulse ml-1"></span>}
        </div>
      </div>
    );
  };
        ...
        <div className="flex-1 flex flex-col md:flex-row p-6 overflow-hidden min-h-0 gap-4">
          {activePinnedId ? (
            ...
            <div className={`grid gap-4 flex-1 ${getGridCols(totalVideoTiles)} auto-rows-fr min-h-0 w-full`}>
              {allTiles.length === 0 ? (
                <div className="flex flex-col items-center justify-center w-full h-full bg-slate-800 rounded-2xl border border-slate-700 shadow-sm col-span-full">
                  ...
                  <h3 className="text-xl font-bold text-white mb-2 tracking-tight">Waiting for Host</h3>
                  <p className="text-sm text-slate-400">The live broadcast will begin shortly...</p>
                </div>
              ) : (
